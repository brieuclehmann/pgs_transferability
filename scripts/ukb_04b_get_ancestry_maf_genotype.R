...
this_df <- pan_ukbb_df %>%
    filter(high_quality & info == 1 & chrom == this_chrom) %>%
  ...
  mutate(varid2 = paste0(chrom, ":", pos, "_", ref, "_", alt)) # to unify rsids and varids

out_rsids <- this_df$varid2
out_dir <- file.path("data", "snp_ids", paste0("ancestry~", ancestry))
out_file <- file.path(out_dir, paste0("chrom~", this_chrom, "_genotyped.txt"))
