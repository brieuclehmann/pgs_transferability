...
basic_files <- paste0("output/ukb/pheno~", 
                      pheno_codes, 
                      "/min_ancestry~AFR/scores_genotyped.tsv")
...
#                position = position_dodge(width = 0.1)) +
  ...
  geom_ribbon(data = filter(plot_df, `Test set` == "EUR" & prop_min == 0),
              ...
              inherit.aes = FALSE) +
 # geom_errorbar(data = filter(plot_df, pop == "EUR" & prop_min == 0),
 #               aes(pow, mean, ymin = min, ymax = max), width = 0,
 #               inherit.aes = FALSE) +
...
ggsave("plots/SI_fig_geno_afr_r2.pdf", width = 8, height = 6)
